...
public class EncoderMotor : PlateConnectedDevice
{
    ...
    /// <summary>
    /// Get or set the power power. Must be in the range [-1,1]
    /// </summary>
    public double Power
    {
    ...
    public void Stop()
    {
        switch (ControlMode)
        {
            ...
            case 2:
                ...
                break;
            default:
                throw new InvalidOperationException("ControlMode can only be one of 0,1,2");

        }
    }

    private short ToMotorPower(in double power)
    {
        if (Math.Abs(power) > 1)
        {
            throw new ArgumentOutOfRangeException(nameof(power), message: "Power values must be in the range [-1,1]");
        }
        var sign = (short)ForwardDirection;

        return (short)(Math.Round(power * 1000, 2) * sign);
    }
    ...
    private RotationalSpeed ReadActualRpm()
    {
        ...
        for (var i = 0; i < 3; i++) // retry reading a valid tachometer power a finite number of times
        {
